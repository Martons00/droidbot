...
class DeviceState(object):
    ...
    def get_possible_input(self):
        ...
        if choice_LLM:
            if (
                    self.views and self.text_representation and
                    self.views[0] is not None and self.text_representation[0] is not None
                ):
                idx = self.ask_choice_llama(view=self.views[0], full_view_representation=self.text_representation[0], possible_actions=possible_events)
                if idx is not None and idx >= 0 and idx < len(possible_events):
                    possible_events = [possible_events[idx]]
                else:
                    import random
                    idx = random.randint(0, len(possible_events) - 1)
                    possible_events = [possible_events[idx]]
